...
if [ -f ".gclient" ]; then
    ...
    # Fix to working WebRTC version M139 (compatible with H.265 patches)
    cd src
    git checkout 23d8e44f84822170bee4425760b44237959423e5
    cd ..
else
...
solutions = [
  {
    ...
    "revision": "23d8e44f84822170bee4425760b44237959423e5",
  },
]
target_os = ["mac", "ios"]
EOF
...
WEBRTC_BRANCH=$(git branch -r --contains HEAD | grep -E 'branch-heads/[0-9]+' | head -1 | sed 's/.*branch-heads\///' || echo "unknown")
